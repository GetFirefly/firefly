...
fn with_greater_local_reference_right_returns_true() {
    is_greater_than(|_, process| process.reference(0), true);
}
...
fn with_same_value_local_reference_right_returns_false() {
    is_greater_than(|_, process| process.reference(1), false);
}

#[test]
fn with_greater_local_reference_right_returns_false() {
    is_greater_than(|_, process| process.reference(2), false);
}
...
{
    super::is_greater_than(|process| process.reference(1), right, expected);
}
