...
struct BranchOpConversion : public EIROpConversion<eir::BranchOp> {
    ...
    LogicalResult matchAndRewrite(
        ...
        rewriter.replaceOpWithNewOp<LLVM::BrOp>(op, operands,
                                                op.getSuccessor());
        return success();
    }
};
...
struct InvokeOpConversion : public EIROpConversion<InvokeOp> {
    ...
    LogicalResult matchAndRewrite(
        ...
        Operation *callOp =
            llvm_invoke(ArrayRef<Type>{}, op.getCalleeAttr(),
                        adaptor.operands(), ok, okArgs, err, errArgs);
        for (auto attr : op.getAttrs()) {
            callOp->setAttr(std::get<Identifier>(attr),
                            std::get<Attribute>(attr));
        }
...
struct LandingPadOpConversion : public EIROpConversion<LandingPadOp> {
    ...
    LogicalResult matchAndRewrite(
        ...
        if (catchClauses.size() != 1)
            return op.emitOpError("expected only a single catch clause");

        // The landing pad returns the structure defined above
        Value obj =
            llvm_landingpad(exceptionTy, /*cleanup=*/false, catchClauses);
        ...
        auto reasonPtr = llvm_gep(termPtrTy, erlangErrorPtr,
                                  ArrayRef<Value>{zero, reasonIdx});
...
struct ThrowOpConversion : public EIROpConversion<ThrowOp> {
    ...
    LogicalResult matchAndRewrite(
        ...
        auto raiseOp = rewriter.create<mlir::CallOp>(
            ...
            ArrayRef<Value>{kind, reason, trace});
        ...
        auto callOp = rewriter.create<mlir::CallOp>(
            op.getLoc(), rewriter.getSymbolRefAttr(symbolName),
            ArrayRef<Type>{}, ValueRange{exception});
        callOp.setAttr("tail", rewriter.getUnitAttr());
...
struct ReceiveDoneOpConversion : public EIROpConversion<ReceiveDoneOp> {
    ...
    LogicalResult matchAndRewrite(
        ...
        rewriter.replaceOpWithNewOp<mlir::CallOp>(
            op, calleeSymbol, ArrayRef<Type>{}, ValueRange{recvRef});
        return success();
    }
};
