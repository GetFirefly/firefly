...
use crate::kernel::Annotated;
use crate::kernel::{self as k, Expr as KExpr};
...
impl<'m> LowerFunctionToCore<'m> {
    ...
    fn lower_test<'a>(
        ...
        match (op.function, args.as_slice()) {
            (
                symbols::IsRecord,
                [_tuple, KExpr::Literal(k::Literal {
                    value: k::Lit::Atom(tag),
                    ..
                }), KExpr::Literal(k::Literal {
                    value: k::Lit::Integer(arity),
                    ..
                })],
            ) => {
                let tag = *tag;
    ...
    fn lower_literal<'a>(
        ...
        match literal.value {
            k::Lit::Atom(value) => Ok(builder.ins().atom(value, span)),
            k::Lit::Integer(Integer::Small(value)) => Ok(builder.ins().int(value, span)),
            k::Lit::Integer(Integer::Big(value)) => Ok(builder.ins().bigint(value, span)),
            k::Lit::Float(value) => Ok(builder.ins().float(value.inner(), span)),
            k::Lit::Nil => Ok(builder.ins().nil(span)),
            k::Lit::Cons(box head, box tail) => {
                let head = self.lower_literal(builder, head)?;
                let tail = self.lower_literal(builder, tail)?;
                Ok(builder.ins().cons(head, tail, span))
            }
            k::Lit::Tuple(mut elements) => {
            ...
            k::Lit::Binary(value) => Ok(builder.ins().bitstring(value, span)),
        }
    }
