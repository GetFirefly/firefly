...
fn with_lesser_small_integer_second_returns_first() {
    max(|_, mut process| (-1).into_process(&mut process), First)
}

#[test]
fn with_greater_small_integer_second_returns_second() {
    max(|_, mut process| 1.into_process(&mut process), Second)
}
...
fn with_lesser_float_second_returns_first() {
    max(|_, mut process| (-1.0).into_process(&mut process), First)
}

#[test]
fn with_greater_float_second_returns_second() {
    max(|_, mut process| 1.0.into_process(&mut process), Second)
}
...
fn with_local_reference_second_returns_second() {
    max(|_, mut process| Term::local_reference(&mut process), Second);
}
...
{
    super::max(|mut process| 0.0.into_process(&mut process), second, which);
}
