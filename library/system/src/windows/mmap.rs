...
pub unsafe fn map(layout: Layout) -> Result<NonNull<u8>, AllocError> {
    ...
    let size = round_up_to_multiple_of(base_size, page_size);
...
pub unsafe fn remap(
    ...
    if new_size < old_size {
        ...
        if diff > page_size {
            ...
            let aligned_addr = round_up_to_multiple_of(extra_addr, page_size);
    ...
    let new_size = round_up_to_multiple_of(new_size, layout.align());
