...
fn with_list_encoding_utf8() {
    ...
    assert_eq!(
        erlang::list_to_atom_1(Term::str_to_char_list("atom", &mut process)),
        Ok(Term::str_to_atom("atom", DoNotCare).unwrap())
    );
    assert_eq!(
        erlang::list_to_atom_1(Term::str_to_char_list("José", &mut process)),
        Ok(Term::str_to_atom("José", DoNotCare).unwrap())
    );
    assert_eq!(
        erlang::list_to_atom_1(Term::str_to_char_list("😈", &mut process)),
        Ok(Term::str_to_atom("😈", DoNotCare).unwrap())
    );
}
...
{
    super::errors_badarg(|mut process| erlang::list_to_atom_1(list(&mut process)));
}
