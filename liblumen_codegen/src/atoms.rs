...
pub fn compile_atom_table(
    ...
    fn insert_atom<'ctx>(builder: &ModuleBuilder<'ctx>, atom: Symbol) -> Result<LLVMValueRef> {
    ...
    let table_global = builder.add_global(entry_ptr_type, "__LUMEN_ATOM_TABLE", Some(table_global_init));
    ...
    let table_size_global = builder.add_global(i64_type, "__LUMEN_ATOM_TABLE_SIZE", Some(table_size_global_init));
    ...
    Ok(Arc::new(CompiledModule::new(NAME.to_string(), Some(path), None)))
}
