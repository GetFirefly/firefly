...
fn with_greater_small_integer_right_returns_true() {
    is_greater_than(|_, mut process| (-1).into_process(&mut process), true);
}
...
fn with_same_value_small_integer_right_returns_false() {
    is_greater_than(|_, mut process| 0.into_process(&mut process), false);
}

#[test]
fn with_greater_small_integer_right_returns_false() {
    is_greater_than(|_, mut process| 1.into_process(&mut process), false);
}
...
fn with_greater_float_right_returns_true() {
    is_greater_than(|_, mut process| (-1.0).into_process(&mut process), true)
}

#[test]
fn with_same_value_float_right_returns_false() {
    is_greater_than(|_, mut process| 0.0.into_process(&mut process), false)
}

#[test]
fn with_greater_float_right_returns_false() {
    is_greater_than(|_, mut process| 1.0.into_process(&mut process), false)
}
...
fn with_local_reference_right_returns_false() {
    is_greater_than(|_, mut process| Term::local_reference(&mut process), false);
}
...
{
    super::is_greater_than(|mut process| 0.into_process(&mut process), right, expected);
}
