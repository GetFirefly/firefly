...
pub fn bsr_2(integer: Term, shift: Term, mut process: &mut Process) -> Result {
    match integer.tag() {
        SmallInteger => {
            ...
            // Rust doesn't support negative shift, so negative right shifts need to be left shifts
            if 0 <= shift_isize {
                let shift_usize = shift_isize as usize;
        ...
        Boxed => {
            ...
            match unboxed_integer.tag() {
                BigInteger => {
                    ...
                    // Rust doesn't support negative shift, so negative right shifts need to be left
                    // shifts
                    let shifted = if 0 <= shift_isize {
                        let shift_usize = shift_isize as usize;
