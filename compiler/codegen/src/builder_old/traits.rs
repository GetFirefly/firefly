...
impl AsValueRef for ConstMap {
    fn as_value_ref(
        ...
        let val =
            unsafe { MLIRBuildConstantMap(builder, loc, self.0.as_ptr(), self.0.len() as c_uint) };
...
impl AsValueRef for BigIntTerm {
    fn as_value_ref(
        ...
        let val = unsafe { MLIRBuildConstantBigInt(builder, loc, s.as_ptr(), width as c_uint) };
...
impl AsAttributeRef for ConstMap {
    fn as_attribute_ref(
        ...
        let val =
            unsafe { MLIRBuildMapAttr(builder, loc, self.0.as_ptr(), self.0.len() as c_uint) };
...
impl AsAttributeRef for BigIntTerm {
    fn as_attribute_ref(
        ...
        let s = CString::new(bi.to_str_radix(10)).unwrap();
        let val = unsafe { MLIRBuildBigIntAttr(builder, loc, s.as_ptr(), width as c_uint) };
