...
fn with_same_value_small_integer_right_returns_true() {
    are_equal_after_conversion(|_, process| 0.into_process(&process), true)
}

#[test]
fn with_different_small_integer_right_returns_false() {
    are_equal_after_conversion(|_, process| 1.into_process(&process), false)
}
...
fn with_same_value_big_integer_right_returns_true() {
    ...
    if f < crate::float::INTEGRAL_MAX {
        super::are_equal_after_conversion(
            |process| f.into_process(&process),
            |_, process| i.into_process(&process),
            true,
        )
    }
}
...
fn with_same_value_float_right_returns_true() {
    are_equal_after_conversion(|_, process| 0.0.into_process(&process), true)
}

#[test]
fn with_different_value_float_right_returns_false() {
    are_equal_after_conversion(|_, process| 1.0.into_process(&process), false)
}
...
{
    super::are_equal_after_conversion(|process| 0.into_process(&process), right, expected);
}
