...
fn with_empty_list_returns_binary() {
    with(|head, process| {
        ...
        assert_eq!(
            ...
            Ok(Term::slice_to_binary(&[0b0000_0001], &process))
        );
    })
}
...
fn with_proper_list_returns_binary() {
    with(|head, process| {
        ...
        assert_eq!(
            erlang::list_to_bitstring_1(iolist, &process),
            Ok(Term::slice_to_binary(&[1, 2], &process))
        );
    })
}
...
fn with_byte_errors_badarg() {
    with_tail_errors_badarg(|process| 2.into_process(&process));
}
...
fn with_heap_binary_returns_binary() {
    with(|head, process| {
        ...
        assert_eq!(
            erlang::list_to_bitstring_1(iolist, &process),
            Ok(Term::slice_to_binary(&[1, 2, 3], &process))
        );
    })
}
...
fn with_subbinary_with_bit_count_0_returns_binary() {
    with(|head, process| {
        let original = Term::slice_to_binary(&[0b0000_0010], &process);
    ...
    F: FnOnce(Term, &Process) -> (),
{
    with_process(|process| {
        let original = Term::slice_to_binary(&[0b0000_0001], &process);
        let head = Term::subbinary(original, 0, 0, 1, 0, &process);
