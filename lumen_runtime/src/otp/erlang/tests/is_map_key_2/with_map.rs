...
fn with_atom_key() {
    with_process(|process| {
        ...
        assert_eq!(erlang::is_map_key_2(key, map, &process), Ok(true.into()));

        let non_key = Term::str_to_atom("non_key", DoNotCare).unwrap();

        assert_eq!(
            erlang::is_map_key_2(non_key, map, &process),
            Ok(false.into())
        );
    });
}
...
fn with_local_reference_key() {
    with_process(|process| {
        ...
        assert_eq!(
            erlang::is_map_key_2(key, map_with_key, &process),
            Ok(true.into())
        );
        ...
        assert_eq!(
            erlang::is_map_key_2(key, map_without_key, &process),
            Ok(false.into())
        );
    });
}
