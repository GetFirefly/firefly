...
pub fn make_lumen_web_document() -> NativeModule {
    ...
    native.add_simple(Atom::try_from_str("create_element").unwrap(), 2, |proc, args| {
        Ok(lumen_web::document::create_element_2::native(proc, args[0], args[1]).unwrap())
    });

    native.add_simple(Atom::try_from_str("create_text_node").unwrap(), 2, |proc, args| {
        Ok(lumen_web::document::create_text_node_2::native(proc, args[0], args[1]).unwrap())
    });

    native.add_simple(Atom::try_from_str("get_element_by_id").unwrap(), 2, |proc, args| {
