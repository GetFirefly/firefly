...
impl AsValueRef for Symbol {
    fn as_value_ref(
        ...
        let val = unsafe { unsafe { MLIRBuildConstantAtom(builder, loc, s.as_ptr(), i) } };
...
impl AsAttributeRef for i64 {
    fn as_attribute_ref(
        ...
        let val = unsafe { unsafe { MLIRBuildIntAttr(builder, loc, *self) } };
...
impl AsAttributeRef for Symbol {
    fn as_attribute_ref(
        ...
        let i = self.as_usize() as u64;
        let s = CString::new(self.as_str().get()).unwrap();
        let val = unsafe { unsafe { MLIRBuildAtomAttr(builder, loc, s.as_ptr(), i) } };
