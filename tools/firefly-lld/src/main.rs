//! Script to invoke the bundled firefly-lld with the correct flavor.
//!
...
//!
//! In Firefly with `-Z gcc-ld=lld` we have gcc or clang invoke firefly-lld. Since there is no way to
//! make gcc/clang pass `-flavor <flavor>` as the first two arguments in the linker invocation
//! and since Windows does not support symbolic links for files this wrapper is used in place of a
//! symbolic link. It execs `../firefly-lld -flavor <flavor>` by propagating the flavor argument
//! obtained from the wrapper's name as the first two arguments.
//! On Windows it spawns a `..\firefly-lld.exe` child process.
...
/// Returns the path to firefly-lld in the parent directory.
///
/// Exits if the parent directory cannot be determined.
fn get_firefly_lld_path(current_exe_path: &Path) -> PathBuf {
    let mut firefly_lld_exe_name = "firefly-lld".to_owned();
    firefly_lld_exe_name.push_str(EXE_SUFFIX);
    let mut firefly_lld_path = current_exe_path
        .parent()
        ...
        .to_owned();
    firefly_lld_path.push(firefly_lld_exe_name);
    firefly_lld_path
}
...
fn get_lld_flavor(current_exe_path: &Path) -> Result<&'static str, String> {
    ...
    let stem = file
        .and_then(|s| s.to_str())
        .map(|s| s.trim_end_matches(EXE_SUFFIX));
    Ok(match stem {
        Some("ld.lld") => "gnu",
...
/// Returns the command for invoking firefly-lld with the correct flavor.
/// LLD only accepts the flavor argument at the first two arguments, so pass it there.
///
/// Exits on error.
fn get_firefly_lld_command(current_exe_path: &Path) -> process::Command {
    let firefly_lld_path = get_firefly_lld_path(current_exe_path);
    let mut command = process::Command::new(firefly_lld_path);
...
fn exec_lld(mut command: process::Command) {
    ...
    Result::<(), _>::Err(command.exec()).unwrap_or_exit_with("could not exec firefly-lld");
...
fn exec_lld(mut command: process::Command) {
    ...
    let exit_status = command
        .status()
        .unwrap_or_exit_with("error running firefly-lld child process");
    let code = exit_status
        .code()
        .ok_or(exit_status)
        .unwrap_or_exit_with("firefly-lld child process exited with error");
...
fn main() {
    ...
    exec_lld(get_firefly_lld_command(current_exe_path.as_ref()));
}
