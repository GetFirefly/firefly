...
pub const MAX_ALIGNED_ADDR: u32 = u32::max_value() & !(MIN_ALIGN as u32- 1);
    ...
    T: Boxable<RawTerm>,
{
    ...
    default fn dyn_cast(self) -> *mut T {
        ...
        (self.0 & !MASK_PRIMARY) as *const T as *mut T
    }
}
...
impl Cast<*mut Cons> for RawTerm {
    ...
    fn dyn_cast(self) -> *mut Cons {
        assert!(self.is_list());
        (self.0 & !MASK_PRIMARY) as *const Cons as *mut Cons
    }
}
