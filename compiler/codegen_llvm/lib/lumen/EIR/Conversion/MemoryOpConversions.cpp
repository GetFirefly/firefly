  ...
  LogicalResult matchAndRewrite(
    ...
    if (auto tt = toTy.dyn_cast_or_null<OpaqueTermType>()) {
      ...
      if (auto llvmFromTy = fromTy.dyn_cast_or_null<LLVMType>()) {
        // Handle converting from a raw pointer to an opaque term or boxed term
        if (llvmFromTy.isPointerTy() && (tt.isOpaque() || tt.isBox())) {
          ...
          // Raw pointer, this kind of cast shouldn't occur anymore, but we can
          // handle it if we have type information, so do so
          if (elTy.isInteger(8)) {
          ...
          if (isCastable) {
            ...
            op.emitError("unsupported cast from llvm type to term type");
            return failure();
          }
        }
