...
impl FunctionContext {
    ...
    fn new_vars<const N: usize>(&mut self, span: Option<SourceSpan>) -> [Var; N] {
        let vars = [0; N];
        vars.map(|_| self.next_var(span))
    }

    fn n_vars(&mut self, n: usize, span: Option<SourceSpan>) -> Vec<Var> {
...
impl Pass for CstToKernel {
    ...
    fn run<'a>(&mut self, mut cst: Self::Input<'a>) -> anyhow::Result<Self::Output<'a>> {
        ...
        while let Some((name, function)) = cst.functions.pop_first() {
            let context = FunctionContext::new(name, &function);

            let mut pipeline = TranslateCst::new(self.reporter.clone(), context);
...
impl TranslateCst {
    ...
    fn body(&mut self, expr: cst::Expr, sub: BiMap) -> Result<(Expr, Vec<Expr>), ExprError> {
        match expr {
            cst::Expr::Values(cst::Values {
                annotations,
                values,
                ..
            }) => {
    ...
    fn guard(&mut self, expr: cst::Expr, sub: BiMap) -> Result<Expr, ExprError> {
    ...
    fn expr(&mut self, expr: cst::Expr, sub: BiMap) -> Result<(Expr, Vec<Expr>), ExprError> {
        match expr {
            cst::Expr::Var(v) if v.arity.is_some() => {
                let span = v.span();
            ...
            cst::Expr::Literal(lit) => Ok((Expr::Literal(lit), vec![])),
            cst::Expr::Cons(cst::Cons {
                span,
                annotations,
                box head,
                box tail,
            ...
            cst::Expr::Binary(cst::Binary {
                ...
                Err(ExprError::BadSegmentSize(span)) => {
                    ...
                    let badarg = cst::Expr::Literal(Literal::atom(span, symbols::Badarg));
                    let error = cst::Call::new(span, symbols::Erlang, symbols::Error, vec![badarg]);
                    self.expr(cst::Expr::Call(error), sub)
                }
            },
            cst::Expr::Fun(cst::Fun {
                span,
                annotations,
                mut vars,
                box body,
                ...
                let cvars = vars.drain(..).map(cst::Expr::Var).collect();
            ...
            cst::Expr::Call(cst::Call {
                ...
                match call_type(&module, &function, args.as_slice()) {
                    CallType::Error => {
                        ...
                        mfa.push(function);
                        mfa.push(argv);
                        let call = cst::Expr::Call(cst::Call::new(
                            span,
                            symbols::Erlang,
                            symbols::Apply,
                            mfa,
            ...
            cst::Expr::Try(cst::Try {
                ...
                let cevars = evars.drain(..).map(cst::Expr::Var).collect();
    ...
    fn translate_match_fail(
        ...
        let (args, annotations) = match arg {
            ...
            reason @ cst::Expr::Literal(_) => (vec![reason], annotations),
    ...
    fn atomic(&mut self, expr: cst::Expr, sub: BiMap) -> Result<(Expr, Vec<Expr>), ExprError> {
    ...
    fn atomic_bin(
        &mut self,
        mut segments: Vec<cst::Bitstring>,
        sub: BiMap,
    ...
    fn variable(&mut self, expr: cst::Expr, sub: BiMap) -> Result<(Expr, Vec<Expr>), ExprError> {
        let (expr, mut pre) = self.expr(expr, sub)?;
    ...
    fn pattern(
        ...
        match pattern {
            ...
            cst::Expr::Literal(lit) => Ok((Expr::Literal(lit), osub)),
            cst::Expr::Cons(cst::Cons {
                span,
                annotations,
                box head,
            ...
            cst::Expr::Alias(cst::Alias {
                ...
                let mut vs = vec![cst::Expr::Var(var)];
                vs.extend(vars.drain(..).map(cst::Expr::Var));
...
fn call_type(module: &cst::Expr, function: &cst::Expr, args: &[cst::Expr]) -> CallType {
    let arity = args.len() as u8;
...
fn same_args(args: &[cst::Expr], fargs: &[Var], sub: &BiMap) -> bool {
...
fn make_clist(mut items: Vec<cst::Expr>) -> cst::Expr {
    items.drain(..).rfold(
        cst::Expr::Literal(Literal::nil(SourceSpan::default())),
        |tail, head| {
            let span = head.span();
            cst::Expr::Cons(cst::Cons::new(span, head, tail))
        },
    )
}
...
fn flatten_alias(expr: cst::Expr) -> (Vec<Var>, cst::Expr) {
    let mut vars = vec![];
    let mut pattern = expr;
    while let cst::Expr::Alias(cst::Alias {
        var,
        pattern: box p,
        ..
    }) = pattern
...
impl TranslateCst {
    ...
    fn ubody(&mut self, body: Expr, brk: Brk) -> Result<(Expr, RedBlackTreeSet<Ident>), ExprError> {
        match body {
            ...
            Expr::Set(ISet {
                ...
                let used = union(au, subtract(bu, ns)); // used external vars
                Ok((
                    Expr::Seq(Seq {
                        span,
                        annotations,
    ...
    fn uexpr(&mut self, expr: Expr, brk: Brk) -> Result<(Expr, RedBlackTreeSet<Ident>), ExprError> {
        match expr {
            ...
            Expr::Set(ISet {
                ...
                let used = union(eu, subtract(bu, ns));
                Ok((
                    Expr::Seq(Seq {
                        span,
                        annotations,
            ...
            Expr::If(If {
                ...
                let used = union(cond_used, union(tbu, ebu));
                Ok((
                    Expr::If(If {
                        span,
                        annotations,
            ...
            Expr::Call(Call {
                ...
                let used = union(op_vars(callee.as_ref()), lit_list_vars(args.as_slice()));
                let enter = Expr::Enter(Enter {
                    span,
                    annotations,
                    callee,
            ...
            Expr::Try(Try {
                ...
                if is_simple {
                    ...
                    let used = union(au, union(subtract(bu, var_set), subtract(hu, evar_set)));
                    Ok((
                        Expr::Try(Try {
                            span,
                            annotations,
            ...
            Expr::Fun(IFun {
                ...
                let ns = vars.iter().map(|v| v.name).collect();
                let free = subtract(bu, ns); // Free variables in fun
                let mut fvs = free
                    .iter()
                    .copied()
            ...
            Expr::LetRecGoto(LetRecGoto {
                ...
                let used = subtract(union(fu, tu), ns);
                Ok((
                    Expr::LetRecGoto(LetRecGoto {
                        span,
                        annotations,
    ...
    fn umatch_value_clause(
        ...
        let used = union(used, subtract(bu, ps));
        Ok((
            ValueClause {
                span: clause.span,
                annotations: clause.annotations,
...
fn pat_anno_unused(pattern: &mut Expr, used: RedBlackTreeSet<Ident>, ps: RedBlackTreeSet<Ident>) {
    match pattern {
        Expr::Tuple(Tuple {
            ...
            // from a huge record.
            let used = super::intersection(used, ps);
...
fn lit_list_vars(patterns: &[Expr]) -> RedBlackTreeSet<Ident> {
    patterns
        .iter()
        .fold(rbt_set![], |vars, p| union(lit_vars(p), vars))
}
...
fn lit_vars(expr: &Expr) -> RedBlackTreeSet<Ident> {
    match expr {
        ...
        Expr::Cons(Cons { head, tail, .. }) => {
            union(lit_vars(head.as_ref()), lit_vars(tail.as_ref()))
        }
        ...
        Expr::BinarySegment(BinarySegment {
            ...
            lit_vars(sz.as_ref()),
            union(lit_vars(value.as_ref()), lit_vars(next.as_ref())),
        ),
...
fn pat_vars(pattern: &Expr) -> (RedBlackTreeSet<Ident>, RedBlackTreeSet<Ident>) {
    match pattern {
        ...
        Expr::Cons(Cons { head, tail, .. }) => {
            ...
            (union(used0, used1), union(new0, new1))
        }
        ...
        Expr::BinarySegment(BinarySegment {
            ...
            let (used, new) = (union(used0, used1), union(new0, new1));
            let (_, used2) = pat_vars(sz.as_ref());
            (union(used, used2), new)
        }
        Expr::BinarySegment(BinarySegment { value, next, .. }) => {
            let (used0, new0) = pat_vars(value.as_ref());
            let (used1, new1) = pat_vars(next.as_ref());
        ...
        Expr::Map(Map { pairs, .. }) => {
            pairs
                ...
                .fold((rbt_set![], rbt_set![]), |(used, new), pair| {
                    ...
                    (union(union(used1, used2), used), union(new1, new))
                })
        }
...
fn pat_list_vars(patterns: &[Expr]) -> (RedBlackTreeSet<Ident>, RedBlackTreeSet<Ident>) {
    patterns
        ...
        .fold((rbt_set![], rbt_set![]), |(used, new), pattern| {
            ...
            (union(used, used1), union(new, new1))
        })
}
