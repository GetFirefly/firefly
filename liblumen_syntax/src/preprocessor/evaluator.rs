...
pub fn eval(expr: Expr) -> Result<Expr, PreprocessorError> {
    let result = match expr {
        ...
        Expr::Tuple(span, elements) => {
            Expr::Tuple(span, eval_list(elements)?)
        }
        Expr::Map(span, Some(box lhs), fields) => {
            Expr::Map(span, Some(Box::new(eval(lhs)?)), eval_map(fields)?)
        }
        Expr::Map(span, None, fields) => {
            Expr::Map(span, None, eval_map(fields)?)
        }
        Expr::Binary(span, elements) => {
            Expr::Binary(span, eval_bin_elements(elements)?)
        }
        ...
        Expr::Record(span, name, fields) => {
            Expr::Record(span, name, eval_record(fields)?)
        }
        Expr::Begin(span, _) => {
            return Err(PreprocessorError::InvalidConstExpression(span));
        }
        Expr::Apply { span, box lhs, args } => {
        ...
        Expr::BinaryExpr { span, box lhs, op, box rhs } => {
            let lhs = eval(lhs)?;
            let rhs = eval(rhs)?;
    ...
    for field in fields.drain(..) {
        match field {
            MapField::Assoc { span, key, value } =>
                result.push(MapField::Assoc { span, key: eval(key)?, value: eval(value)? }),
            MapField::Exact { span, key, value } =>
                result.push(MapField::Exact { span, key: eval(key)?, value: eval(value)? }),
        }
    }
...
fn eval_record(mut fields: Vec<RecordField>) -> Result<Vec<RecordField>, PreprocessorError> {
    ...
    for field in fields.drain(..) {
        let new_field = match field {
            RecordField { span, name, value: Some(value), ty } =>
                RecordField { span, name, value: Some(eval(value)?), ty },
            RecordField { span, name, value: None, ty } =>
                RecordField { span, name, value: None, ty },
        };
...
fn eval_bin_elements(mut elements: Vec<BinaryElement>) -> Result<Vec<BinaryElement>, PreprocessorError> {
    let mut result = Vec::new();

    for element in elements.drain(..) {
        let new_element = match element {
            BinaryElement { span, bit_expr, bit_size: Some(bit_size), bit_type } =>
                BinaryElement { span, bit_expr: eval(bit_expr)?, bit_size: Some(eval(bit_size)?), bit_type },

            BinaryElement { span, bit_expr, bit_size: None, bit_type } =>
                BinaryElement { span, bit_expr: eval(bit_expr)?, bit_size: None, bit_type }
        };
...
fn eval_binary_op(span: ByteSpan, lhs: Expr, op: BinaryOp, rhs: Expr) -> Result<Expr, PreprocessorError> {
    match op {
        BinaryOp::OrElse | BinaryOp::AndAlso | BinaryOp::Or | BinaryOp::And =>
            eval_boolean(span, lhs, op, rhs),
        BinaryOp::Equal | BinaryOp::NotEqual =>
            eval_equality(span, lhs, op, rhs),
        ...
        BinaryOp::Add | BinaryOp::Sub | BinaryOp::Multiply | BinaryOp::Divide | BinaryOp::Div | BinaryOp::Rem =>
            eval_arith(span, lhs, op, rhs),
        BinaryOp::Bor | BinaryOp::Bxor | BinaryOp::Xor | BinaryOp::Band | BinaryOp::Bsl | BinaryOp::Bsr =>
            eval_shift(span, lhs, op, rhs),
        _ =>
            return Err(PreprocessorError::InvalidConstExpression(span)),
    }
}
        ...
        UnaryOp::Not =>
            match rhs {
                Expr::Literal(Literal::Atom(Ident { name, span })) if name == symbols::True =>
                    Expr::Literal(Literal::Atom(Ident { name: symbols::False, span })),
                Expr::Literal(Literal::Atom(Ident { name, span })) if name == symbols::False =>
                    Expr::Literal(Literal::Atom(Ident { name: symbols::True, span })),
                _ =>
                    return Err(PreprocessorError::InvalidConstExpression(span)),
            }
    };
...
fn eval_boolean(span: ByteSpan, lhs: Expr, op: BinaryOp, rhs: Expr) -> Result<Expr, PreprocessorError> {
    ...
    match op {
        BinaryOp::Xor => {
            if (left != right) && (left || right) {
                return Ok(Expr::Literal(Literal::Atom(Ident { name: symbols::True, span })));
            }
            return Ok(Expr::Literal(Literal::Atom(Ident { name: symbols::False, span })));
        }
...
fn eval_equality(span: ByteSpan, lhs: Expr, op: BinaryOp, rhs: Expr) -> Result<Expr, PreprocessorError> {
    if is_number(&lhs) && is_number(&rhs) {
        ...
        match op {
            BinaryOp::Equal =>
                if lhs == rhs {
                    Ok(Expr::Literal(Literal::Atom(Ident { name: symbols::True, span })))
                } else {
                    Ok(Expr::Literal(Literal::Atom(Ident { name: symbols::False, span })))
                },
            BinaryOp::NotEqual =>
                if lhs != rhs {
                    Ok(Expr::Literal(Literal::Atom(Ident { name: symbols::True, span })))
...
fn eval_strict_equality(span: ByteSpan, lhs: Expr, op: BinaryOp, rhs: Expr) -> Result<Expr, PreprocessorError> {
    match op {
...
fn eval_numeric_equality(span: ByteSpan, lhs: Expr, op: BinaryOp, rhs: Expr) -> Result<Expr, PreprocessorError> {
    let result = match (lhs, rhs) {
        (Expr::Literal(Literal::Integer(_, x)), Expr::Literal(Literal::Integer(_, y))) =>
            match op {
                BinaryOp::Equal if x == y =>
                    Expr::Literal(Literal::Atom(Ident { name: symbols::True, span })),
                BinaryOp::NotEqual if x != y =>
                    Expr::Literal(Literal::Atom(Ident { name: symbols::True, span })),
                BinaryOp::Equal =>
                    Expr::Literal(Literal::Atom(Ident { name: symbols::False, span })),
                BinaryOp::NotEqual =>
        ...
        (Expr::Literal(Literal::Integer(xspan, x)), rhs @ Expr::Literal(Literal::BigInteger(_, _))) =>
            return eval_numeric_equality(span, Expr::Literal(Literal::BigInteger(xspan, Integer::from(x))), op, rhs),

        (Expr::Literal(Literal::Integer(xspan, x)), rhs @ Expr::Literal(Literal::Float(_, _))) =>
            return eval_numeric_equality(span, Expr::Literal(Literal::Float(xspan, x as f64)), op, rhs),

        (lhs @ Expr::Literal(Literal::BigInteger(_, _)), Expr::Literal(Literal::Integer(yspan, y))) =>
            return eval_numeric_equality(span, lhs, op, Expr::Literal(Literal::BigInteger(yspan, Integer::from(y)))),

        (lhs @ Expr::Literal(Literal::Float(_, _)), Expr::Literal(Literal::Integer(yspan, y))) =>
            return eval_numeric_equality(span, lhs, op, Expr::Literal(Literal::Float(yspan, y as f64))),
...
fn eval_comparison(span: ByteSpan, lhs: Expr, op: BinaryOp, rhs: Expr) -> Result<Expr, PreprocessorError> {
    match op {
...
fn eval_arith(span: ByteSpan, lhs: Expr, op: BinaryOp, rhs: Expr) -> Result<Expr, PreprocessorError> {
    if is_number(&lhs) && is_number(&rhs) {
        let result = match (lhs, rhs) {
            // Types match
            (Expr::Literal(Literal::Integer(_, x)), Expr::Literal(Literal::Integer(_, y))) =>
...
fn eval_op_int(span: ByteSpan, x: i64, op: BinaryOp, y: i64) -> Result<Expr, PreprocessorError> {
    let result = match op {
        BinaryOp::Add =>
            Expr::Literal(Literal::Integer(span, x + y)),
        BinaryOp::Sub =>
            Expr::Literal(Literal::Integer(span, x - y)),
        BinaryOp::Multiply =>
            Expr::Literal(Literal::Integer(span, x * y)),
        BinaryOp::Divide if y == 0 =>
            return Err(PreprocessorError::InvalidConstExpression(span)),
        BinaryOp::Divide =>
            Expr::Literal(Literal::Float(span, (x as f64) / (y as f64))),
        BinaryOp::Div if y == 0 =>
            return Err(PreprocessorError::InvalidConstExpression(span)),
        BinaryOp::Div =>
            Expr::Literal(Literal::Integer(span, x / y)),
...
fn eval_op_bigint(span: ByteSpan, x: Integer, op: BinaryOp, y: Integer) -> Result<Expr, PreprocessorError> {
    let zero = Integer::new();
    let result = match op {
        BinaryOp::Add =>
            Expr::Literal(Literal::BigInteger(span, x + y)),
        BinaryOp::Sub =>
            Expr::Literal(Literal::BigInteger(span, x - y)),
        BinaryOp::Multiply =>
            Expr::Literal(Literal::BigInteger(span, x * y)),
...
fn eval_op_float(span: ByteSpan, x: f64, op: BinaryOp, y: f64) -> Result<Expr, PreprocessorError> {
      match op {
        BinaryOp::Add =>
            Ok(Expr::Literal(Literal::Float(span, x + y))),
        BinaryOp::Sub =>
            Ok(Expr::Literal(Literal::Float(span, x - y))),
        BinaryOp::Multiply =>
            Ok(Expr::Literal(Literal::Float(span, x * y))),
...
fn eval_shift(span: ByteSpan, lhs: Expr, op: BinaryOp, rhs: Expr) -> Result<Expr, PreprocessorError> {
    match (lhs, rhs) {
        (Expr::Literal(Literal::Integer(_, x)), Expr::Literal(Literal::Integer(_, y))) => {
            let result = match op {
                BinaryOp::Bor => {
