...
struct ClosureOpConversion : public EIROpConversion<ClosureOp> {
    ...
    LogicalResult matchAndRewrite(
        ...
        Value headerArityConst =
            llvm_constant(immedTy, ctx.getIntegerAttr(headerArity));
        auto mallocOp =
        ...
        Value header = llvm_constant(
            immedTy, ctx.getIntegerAttr(closureHeader.getLimitedValue()));
        Value headerPtrGep =
            llvm_gep(immedTy.getPointerTo(), valRef, ValueRange{zero, zero});
        ...
        Value arityPtrGep =
            llvm_gep(i32PtrTy, valRef, ValueRange{zero, arityIdx});
        ...
        if (isAnonymous) {
            ...
            // Definition - unique
            Value uniqueConst =
                llvm_constant(uniqueTy, ctx.getStringAttr(unique));
            Value defUniqIdx = llvm_constant(i32Ty, ctx.getI32Attr(2));
            Value defUniqGep = llvm_gep(uniquePtrTy, valRef,
                                        ValueRange{zero, defIdx, defUniqIdx});
            llvm_store(uniqueConst, defUniqGep);

            // Definition - old_unique
            Value oldUniqueConst =
                llvm_constant(i32Ty, ctx.getI32Attr(oldUnique));
            Value defOldUniqIdx = llvm_constant(i32Ty, ctx.getI32Attr(3));
            Value defOldUniqGep = llvm_gep(
                i32PtrTy, valRef, ValueRange{zero, defIdx, defOldUniqIdx});
        ...
        if (isAnonymous) {
            ...
            if (opOperands.size() > envLen)
                return op.emitOpError(
                           "mismatched closure env signature, expected ")
                       << envLen << ", got " << opOperands.size();

            if (opOperands.size() > 0) {
                Value envIdx =
                    llvm_constant(i32Ty, ctx.getI32Attr(CLOSURE_ENV_INDEX));

                for (auto it : llvm::enumerate(opOperands)) {
                    Value operand = it.value();
                    Value opIdx =
                        llvm_constant(i32Ty, ctx.getI32Attr(it.index()));
                    Value opPtrGep = llvm_gep(termPtrTy, valRef,
                                              ValueRange{zero, envIdx, opIdx});
...
struct UnpackEnvOpConversion : public EIROpConversion<UnpackEnvOp> {
    ...
    LogicalResult matchAndRewrite(
        ...
        Value ptr =
            llvm_gep(termPtrTy, env, ValueRange{zero, fieldIdx, envIdx});
