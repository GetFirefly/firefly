...
fn with_subbinary_errors_badmap() {
    with_key_and_map_errors_badmap(
        |process| {
            // <<3::3, 4>>
            let key_original = Term::slice_to_binary(&[96, 0b0100_0000], &process);
            Term::subbinary(key_original, 0, 1, 1, 0, &process)
        },
        |process| {
            // <<1::1, 2>>
            let map_original = Term::slice_to_binary(&[96, 0b0100_0000], &process);
            Term::subbinary(map_original, 0, 1, 1, 0, &process)
        },
    );
}
    ...
    M: FnOnce(&Process) -> Term,
{
    with_process(|process| {
        ...
        assert_badmap!(erlang::is_map_key_2(key, map, &process), map, &process);
    });
}
