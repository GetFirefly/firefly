...
fn with_same_value_small_integer_right_returns_true() {
    is_equal_or_less_than(|_, process| 0.into_process(&process), true);
}
...
fn with_same_value_float_right_returns_true() {
    is_equal_or_less_than(|_, process| 0.0.into_process(&process), true)
}
...
{
    super::is_equal_or_less_than(|process| 0.into_process(&process), right, expected);
}
