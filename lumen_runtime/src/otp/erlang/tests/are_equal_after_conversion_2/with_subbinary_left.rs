...
fn with_same_value_subbinary_right_returns_true() {
    are_equal_after_conversion(|_, process| bitstring!(1 :: 1, &process), true);
}

#[test]
fn with_different_subbinary_right_returns_false() {
    are_equal_after_conversion(|_, process| bitstring!(2 :: 2, &process), false);
}
...
{
    super::are_equal_after_conversion(|process| bitstring!(1 :: 1, &process), right, expected);
}
