...
fn returns_different_reference_each_time() {
    with_process_arc(|monitoring_arc_process| {
        ...
        let first_monitor_reference =
            native(&monitoring_arc_process, r#type(), identifier).unwrap();
...
fn when_monitored_process_exits_it_sends_message_for_each_monitor_reference() {
    with_process_arc(|monitoring_arc_process| {
        ...
        let first_identifier = monitoring_arc_process
            ...
            .unwrap();
        let first_monitor_reference =
            native(&monitoring_arc_process, r#type(), first_identifier).unwrap();
        ...
        let second_identifier = monitoring_arc_process
            ...
            .unwrap();
        let second_monitor_reference =
            native(&monitoring_arc_process, r#type(), second_identifier).unwrap();
