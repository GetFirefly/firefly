...
fn with_proper_list_returns_binary() {
    with(|head, process| {
        ...
        assert_eq!(
            ...
            Ok(process.binary_from_bytes(&[1, 2]))
        );
    })
}
...
fn with_subbinary_with_bit_count_0_returns_binary() {
    with(|head, process| {
        let original = process.binary_from_bytes(&[0b0000_0010]);
    ...
    F: FnOnce(Term, &Process) -> (),
{
    with_process(|process| {
        let original = process.binary_from_bytes(&[0b0000_0001]);
        let head = process.subbinary_from_original(original, 0, 0, 1, 0);
