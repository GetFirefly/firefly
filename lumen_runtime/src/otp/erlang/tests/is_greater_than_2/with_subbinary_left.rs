...
fn with_map_right_returns_true() {
    is_greater_than(|_, mut process| Term::slice_to_map(&[], &mut process), true);
}
...
fn with_same_value_subbinary_right_returns_false() {
    is_greater_than(|_, mut process| bitstring!(1, 1 :: 2, &mut process), false);
}
...
fn with_subbinary_with_value_with_shorter_length_returns_false() {
    is_greater_than(|_, mut process| bitstring!(1, 1 :: 1, &mut process), false)
}
