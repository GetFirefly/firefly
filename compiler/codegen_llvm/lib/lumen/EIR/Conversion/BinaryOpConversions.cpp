...
struct BinaryPushOpConversion : public EIROpConversion<BinaryPushOp> {
    ...
    LogicalResult matchAndRewrite(
        ...
        case BinarySpecifierType::Bytes: {
            ...
            if (sizeOpt != nullptr) {
                StringRef symbolName(
                    "__lumen_builtin_binary_push_byte_size_unit");
                // __lumen_builtin_binary_push_byte_size_unit(bin, value, size,
                // unit)
        ...
        case BinarySpecifierType::Bits: {
            ...
            if (sizeOpt != nullptr) {
                StringRef symbolName(
                    "__lumen_builtin_binary_push_bits_size_unit");
                // __lumen_builtin_binary_push_bits_size_unit(bin, value, size,
                // unit)
                auto callee = ctx.getOrInsertFunction(
                    symbolName, pushTy, {termTy, termTy, termTy, i8Ty});
                auto calleeSymbol =
        ...
        case BinarySpecifierType::Integer: {
            ...
            // __lumen_builtin_binary_push_integer(bin, value, size, unit,
            // signed, endianness)
            auto callee = ctx.getOrInsertFunction(
                symbolName, pushTy,
                {termTy, termTy, termTy, i8Ty, i1Ty, i32Ty});
        ...
        Value newBin =
            llvm_extractvalue(termTy, result, ctx.getI64ArrayAttr(0));
        Value successFlag =
            llvm_extractvalue(i1Ty, result, ctx.getI64ArrayAttr(1));
...
class BinaryMatchOpConversion : public EIROpConversion<Op> {
   public:
    ...
    LogicalResult matchAndRewrite(
        ...
        // Obtain the result values from the match result structure and map them
        // to the op outputs
        ...
        Value successFlag =
            llvm_extractvalue(i1Ty, result, ctx.getI64ArrayAttr(2));
...
struct BinaryMatchRawOpConversion
    ...
    void addExtraArgValues(BinaryMatchRawOp &op,
        ...
        Value unit = llvm_constant(
            i8Ty,
            ctx.getIntegerAttr(op.unitAttr().getValue().getLimitedValue()));
        args.push_back(unit);
    }
};
...
struct BinaryMatchIntegerOpConversion
    ...
    void addExtraArgValues(BinaryMatchIntegerOp &op,
        ...
        Value isSigned = llvm_constant(
            i1Ty, ctx.getIntegerAttr(op.isSignedAttr().getValue()));
        Value endianness = llvm_constant(
            usizeTy, ctx.getIntegerAttr(
                         op.endiannessAttr().getValue().getLimitedValue()));
        Value unit = llvm_constant(
            i8Ty,
            ctx.getIntegerAttr(op.unitAttr().getValue().getLimitedValue()));
