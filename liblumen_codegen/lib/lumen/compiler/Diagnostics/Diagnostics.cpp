...
DEFINE_SIMPLE_CONVERSION_FUNCTIONS(mlir::DiagnosticEngine, MLIRDiagnosticEngineRef);
...
extern "C" void
LLVMLumenWriteSMDiagnosticToString(LLVMSMDiagnosticRef D,
                                   RustStringRef Str) {
...
extern "C" void
LLVMLumenWriteDiagnosticInfoToString(LLVMDiagnosticInfoRef DI,
                                     RustStringRef Str) {
  RawRustStringOstream OS(Str);
...
extern "C" void
LLVMLumenUnpackOptimizationDiagnostic(LLVMDiagnosticInfoRef DI,
...
extern "C" MLIRDiagnosticEngineRef
MLIRGetDiagnosticEngine(MLIRContextRef ctx) {
  mlir::DiagnosticEngine &engine = unwrap(ctx)->getDiagEngine();
  return wrap(&engine);
}

extern "C" void
MLIRRegisterDiagnosticHandler(MLIRContextRef ctx, void *handler,
...
extern "C" void
MLIRWriteDiagnosticInfoToString(MLIRDiagnosticRef d, RustStringRef str) {
  RawRustStringOstream OS(str);
  unwrap(d)->print(OS);
}
