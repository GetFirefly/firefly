...
impl AnnotateVarUsage {
    ...
    fn do_uclause(
        ...
        let used = union(used.clone(), intersection(pvs.clone(), known.get().clone()));
        let new = subtract(pvs.clone(), used.clone());
        let known1 = known.union(&new);
        let guards = self.uguard(pguards, clause.guards, known1.clone())?;
        let used_in_guard = used_in_any(guards.iter());
        let new_in_guard = new_in_any(guards.iter());
        ...
        let used = intersection(
            union(used, union(used_in_guard, used_in_any(body.iter()))),
            known.get().clone(),
        );
        let new = union(new, union(new_in_guard, new_in_any(body.iter())));
        let clause = IClause {
            span: clause.span,
            annotations,
            patterns,
                ...
                IExpr::Match(IMatch {
                    ...
                    match *pattern {
                        ...
                        pattern if is_last => {
                            ...
                            let (la0, mut lps) =
                                super::translate::force_safe(self.context_mut(), *arg);
    ...
    fn uexpr(&mut self, expr: IExpr, known: Known) -> anyhow::Result<IExpr> {
        match expr {
            ...
            IExpr::LetRec(ILetRec {
                ...
                let used = used_in_any(defs.iter().map(|(_, expr)| expr).chain(b1.iter()));
            ...
            IExpr::Case(ICase {
                ...
                let used = union(used_in_any(args.iter()), used_in_any(clauses.iter()));
                let new = if annotations.contains(symbols::ListComprehension) {
                    rbt_set![]
                } else {
                    new_in_all(clauses.iter())
                };
                annotations.insert_mut(symbols::Used, used);
            ...
            IExpr::Fun(fun) => {
                ...
                let used_in_clauses = used_in_any(clauses.iter());
                let used = intersection(used_in_clauses, known1.get().clone());
                let used = subtract(used, avs.clone());
                annotations.insert_mut(symbols::Used, used);
            ...
            IExpr::Try(texpr) => {
                ...
                let args_new = new_in_any(args.iter());
                ...
                let used = used_in_any(
                    body.iter()
                        .chain(core::iter::once(&handler))
                        .chain(args.iter()),
                );
                let used = intersection(used, known.get().clone());
                annotations.insert_mut(symbols::Used, used);
                annotations.insert_mut(symbols::New, rbt_set![]);
                Ok(IExpr::Try(ITry {
                    span,
            ...
            IExpr::Catch(ICatch {
                ...
                annotations.insert_mut(symbols::Used, used_in_any(body.iter()));
            ...
            IExpr::Receive1(recv1) => {
                ...
                annotations.insert_mut(symbols::Used, used_in_any(clauses.iter()));
                annotations.insert_mut(symbols::New, new_in_all(clauses.iter()));
            ...
            IExpr::Receive2(recv2) => {
                ...
                let used = union(
                    used_in_any(clauses.iter()),
                    union(used_in_any(action.iter()), timeout.used_vars()),
                );
                let new = if clauses.is_empty() {
                    new_in_any(action.iter())
                } else {
                    intersection(new_in_all(clauses.iter()), new_in_any(action.iter()))
                };
                annotations.insert_mut(symbols::Used, used);
                annotations.insert_mut(symbols::New, new);
            ...
            IExpr::Protect(IProtect {
                ...
                let used = used_in_any(body.iter());
                annotations.insert_mut(symbols::Used, used);
    ...
    fn upattern_list(
        ...
        for pattern in patterns.drain(..) {
            ...
            out.push(p1);
            guard.append(&mut pg);
            vars = union(pv, vars.clone());
            used = union(pu, used.clone());
        }
    ...
    fn upattern(
        ...
        match pattern {
            ...
            IExpr::Cons(ICons {
                ...
                hg.append(&mut tg);
                Ok((cons, hg, union(hv, tv), union(hu, tu)))
            }
            IExpr::Tuple(ITuple {
                span,
                annotations,
    ...
    fn upattern_map(
        ...
        for IMapPair { op, key, box value } in pairs.drain(..) {
            ...
            tests.append(&mut vg);
            pv = union(pv.clone(), vn);
            pu = union(pu.clone(), union(used, vu));
        }
        Ok((out, tests, pv, pu))
    }
    ...
    fn upattern_bin(
        ...
        let us = intersection(vars.clone(), used.clone());
        let used = subtract(used, us);
        Ok((out, guard, vars, used))
    }
...
fn used_in_expr(exprs: &[IExpr]) -> RedBlackTreeSet<Ident> {
    exprs.iter().rfold(RedBlackTreeSet::new(), |used, expr| {
        union(expr.used_vars(), subtract(used, expr.new_vars()))
    })
}
