...
fn with_atom_key() {
    ...
    assert_eq_in_process!(
        erlang::is_map_key(key, map, &mut process),
    ...
    assert_eq_in_process!(
        erlang::is_map_key(non_key, map, &mut process),
...
fn with_empty_key_list() {
    ...
    assert_eq_in_process!(
        erlang::is_map_key(key, map_with_key, &mut process),
        Ok(true.into_process(&mut process)),
        process
    );
    ...
    assert_eq_in_process!(
        erlang::is_map_key(key, map_without_key, &mut process),
        Ok(false.into_process(&mut process)),
        process
    )
}
