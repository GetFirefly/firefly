...
fn with_same_value_small_integer_right_returns_false() {
    are_exactly_not_equal(|_, mut process| 0.into_process(&mut process), false)
}

#[test]
fn with_different_small_integer_right_returns_true() {
    are_exactly_not_equal(|_, mut process| 1.into_process(&mut process), true)
}

#[test]
fn with_same_value_big_integer_right_returns_true() {
    super::are_exactly_not_equal(
        |mut process| ((crate::integer::small::MAX + 1) as f64).into_process(&mut process),
...
fn with_different_value_float_right_returns_true() {
    are_exactly_not_equal(|_, mut process| 1.0.into_process(&mut process), true)
}
...
{
    super::are_exactly_not_equal(|mut process| 0.into_process(&mut process), right, expected);
}
