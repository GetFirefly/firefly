...
pub extern "C-unwind" fn builtin_fail(reason: Term) -> Term {
    if reason.is_none() {
        reason
    } else {
...
pub extern "C-unwind" fn builtin_trace_capture() -> *mut Trace {
    let trace = Trace::capture();
    Trace::into_raw(trace)
}

#[export_name = "__lumen_builtin_trace.print"]
pub extern "C-unwind" fn builtin_trace_print(
    kind: Term,
    reason: Term,
    trace: &mut Trace,
) -> *mut Trace {
...
pub extern "C-unwind" fn builtin_trace_construct(trace: &mut Trace) -> Term {
