...
pub trait Chunk {
    ...
    fn decode<R: Read>(mut reader: R) -> anyhow::Result<Self>
    where
        Self: Sized,
    {
    ...
    fn encode<W: Write>(&self, mut writer: W) -> anyhow::Result<()> {
        let mut buf = Vec::new();
...
impl Chunk for RawChunk {
    ...
    ///   L34-L52) in Kotlin
    fn decode_data<R: Read>(id: &ChunkId, mut reader: R) -> anyhow::Result<Self>
    where
        Self: Sized,
    {
        let mut buf = Vec::new();
    ...
    fn encode_data<W: Write>(&self, mut writer: W) -> anyhow::Result<()> {
