...
pub fn make_liblumen_web_window() -> NativeModule {
    ...
    native.add_simple(Atom::try_from_str("window").unwrap(), 0, |proc, args| {
        ...
        Ok(liblumen_web::window::window_0::native(proc).unwrap())
    });

    native.add_simple(Atom::try_from_str("document").unwrap(), 1, |proc, args| {
        assert_eq!(args.len(), 1);
        Ok(liblumen_web::window::document_1::native(proc, args[0]).unwrap())
    });
