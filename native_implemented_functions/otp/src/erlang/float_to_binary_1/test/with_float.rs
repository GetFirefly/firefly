    ...
    with_process_arc(|arc_process| {
        TestRunner::new(Config::with_source_file(file!()))
            .run(&any::<f64>(), |float_f64| {
                ...
                let result = native(&arc_process, float);
...
fn is_the_same_as_float_to_binary_2_with_scientific_20() {
    with_process_arc(|arc_process| {
        assert_eq!(
            native(&arc_process, arc_process.float(0.0).unwrap()),
            Ok(arc_process
                .binary_from_str("0.00000000000000000000e+00")
                .unwrap())
        );
        assert_eq!(
            native(&arc_process, arc_process.float(0.1).unwrap()),
            Ok(arc_process
    ...
    with_process_arc(|arc_process| {
        TestRunner::new(Config::with_source_file(file!()))
            .run(&strategy::term::float(arc_process.clone()), |float| {
                let result_binary = native(&arc_process, float);

                prop_assert!(result_binary.is_ok());

                let binary = result_binary.unwrap();

                prop_assert_eq!(binary_to_float_1::native(&arc_process, binary), Ok(float));
