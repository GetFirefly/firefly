...
extern "C" bool lumen_is_type(Encoding *encoding, uint32_t type, uint64_t value);
extern "C" uint64_t lumen_encode_immediate(Encoding *encoding, uint32_t type, uint64_t value);
extern "C" uint64_t lumen_encode_header(Encoding *encoding, uint32_t type, uint64_t arity);
...
TargetInfo::TargetInfo(llvm::TargetMachine *targetMachine, LLVMDialect &dialect)
    : archType(targetMachine->getTargetTriple().getArch()),
      pointerSizeInBits(targetMachine->createDataLayout().getPointerSizeInBits(0)),
    ...
    auto termTy = intNTy;
    impl->termTy = termTy;
    impl->atomTy = LLVMType::createStructTy(&dialect, ArrayRef<LLVMType>(intNTy), StringRef("atom"));
    impl->nilTy = LLVMType::createStructTy(&dialect, ArrayRef<LLVMType>(intNTy), StringRef("nil"));
    impl->noneTy = LLVMType::createStructTy(&dialect, ArrayRef<LLVMType>(intNTy), StringRef("none"));
    impl->fixnumTy = LLVMType::createStructTy(&dialect, ArrayRef<LLVMType>(intNTy), StringRef("fixnum"));
    impl->bigIntTy = LLVMType::createStructTy(&dialect, ArrayRef<LLVMType>(intNTy), StringRef("bigint"));
    if (!supportsNanboxing) {
        // Packed floats
        impl->floatTy = LLVMType::createStructTy(&dialect, ArrayRef<LLVMType>({intNTy, f64Ty}), StringRef("float"));
    } else {
        // Immediate floats
        impl->floatTy = f64Ty;
    }
    impl->consTy = LLVMType::createStructTy(&dialect, ArrayRef<LLVMType>({termTy, termTy}), StringRef("cons"));
...
LLVMType TargetInfo::makeTupleType(LLVMDialect *dialect, ArrayRef<LLVMType> elementTypes) {
...
APInt TargetInfo::encodeImmediate(unsigned type, uint64_t value) {
    ...
    auto encoded = lumen_encode_immediate(&impl->encoding, static_cast<uint32_t>(t), value);
    return APInt(pointerSizeInBits, encoded, /*signed=*/false);
}

APInt TargetInfo::encodeHeader(unsigned type, uint64_t arity) {
    auto t = type - mlir::Type::FIRST_EIR_TYPE;
    auto encoded = lumen_encode_header(&impl->encoding, static_cast<uint32_t>(t), arity);
    return APInt(pointerSizeInBits, encoded, /*signed=*/false);
}
