...
/// Tests `value` to see if it holds a function, and puts the result in `dest`
#[derive(Debug, Copy, Clone, PartialEq, Eq)]
...
impl<A: Atom> Opcode<A> {
    ...
    fn tag(&self) -> u8 {
        match self {
            ...
            Self::RecvPop(_) => 102,
            Self::Yield(_) => 103,
            Self::GarbageCollect(_) => 104,
            Self::NormalExit(_) => 105,
            Self::ContinueExit(_) => 106,
            Self::Exit1(_) => 107,
            Self::Exit2(_) => 108,
            Self::Error1(_) => 109,
            Self::Throw1(_) => 110,
            ...
            Self::BsPush(_) => 113,
            Self::BsFinish(_) => 114,
            Self::BsMatchStart(_) => 115,
            Self::BsMatch(_) => 116,
            Self::BsMatchSkip(_) => 117,
            Self::BsTestTail(_) => 118,
            Self::FuncInfo(_) => 119,
            Self::Identity(_) => 120,
            Self::Spawn2(_) => 121,
            Self::Spawn3(_) => 122,
            Self::Spawn3Indirect(_) => 123,
        }
    }
}
...
impl<A: Atom, T: AtomTable<Atom = A>> Decode<A, T> for Opcode<A> {
    fn decode(reader: &mut BytecodeReader<A, T>) -> Result<Self, ReadError<T>> {
        match reader.read_byte()? {
            ...
            103 => Ok(Self::Yield(Yield)),
            104 => Ok(Self::GarbageCollect(Decode::decode(reader)?)),
            105 => Ok(Self::NormalExit(Decode::decode(reader)?)),
            106 => Ok(Self::ContinueExit(Decode::decode(reader)?)),
            107 => Ok(Self::Exit1(Decode::decode(reader)?)),
            108 => Ok(Self::Exit2(Decode::decode(reader)?)),
            109 => Ok(Self::Error1(Decode::decode(reader)?)),
            110 => Ok(Self::Throw1(Decode::decode(reader)?)),
            111 => Ok(Self::Halt(Decode::decode(reader)?)),
            112 => Ok(Self::BsInit(Decode::decode(reader)?)),
            113 => Ok(Self::BsPush(Decode::decode(reader)?)),
            114 => Ok(Self::BsFinish(Decode::decode(reader)?)),
            115 => Ok(Self::BsMatchStart(Decode::decode(reader)?)),
            116 => Ok(Self::BsMatch(Decode::decode(reader)?)),
            117 => Ok(Self::BsMatchSkip(Decode::decode(reader)?)),
            118 => Ok(Self::BsTestTail(Decode::decode(reader)?)),
            119 => Ok(Self::FuncInfo(Decode::decode(reader)?)),
            120 => Ok(Self::Identity(Decode::decode(reader)?)),
            121 => Ok(Self::Spawn2(Decode::decode(reader)?)),
            122 => Ok(Self::Spawn3(Decode::decode(reader)?)),
            123 => Ok(Self::Spawn3Indirect(Decode::decode(reader)?)),
            _ => Err(ReadError::Invalid),
        }
    }
}
