...
fn with_negative_without_underflow_shifts_right() {
    with_process(|mut process| {
        let integer = 0b101100111000.into_process(&mut process);
        let shift = (-9).into_process(&mut process);

        assert_eq!(
            erlang::bsl_2(integer, shift, &mut process),
            Ok(0b101.into_process(&mut process))
        );
    });
}
...
fn with_negative_with_underflow_returns_zero() {
    with_process(|mut process| {
        let integer = 0b101100111000.into_process(&mut process);
        let shift = (-12).into_process(&mut process);

        assert_eq!(
            erlang::bsl_2(integer, shift, &mut process),
            Ok(0b0.into_process(&mut process))
        );
    });
}
...
fn with_zero_returns_same_small_integer() {
    with(|integer, mut process| {
        assert_eq!(
            erlang::bsl_2(integer, 0.into_process(&mut process), &mut process),
            Ok(integer)
        );
    });
}
...
fn with_positive_without_overflow_returns_small_integer() {
    with_process(|mut process| {
        let integer = 0b1.into_process(&mut process);
        let shift = 1.into_process(&mut process);

        let result = erlang::bsl_2(integer, shift, &mut process);
        ...
        assert_eq!(shifted, 0b10.into_process(&mut process));
    })
}
...
{
    with_process(|mut process| {
        let integer = 2.into_process(&mut process);
    ...
    S: FnOnce(&mut Process) -> Term,
{
    super::errors_badarith(|mut process| {
        let integer: Term = 2.into_process(&mut process);
