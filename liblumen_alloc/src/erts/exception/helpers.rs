...
use super::{Exception, RuntimeException, Location};
    ...
    match process.tuple_from_slice(&[fun, args]) {
        Ok(fun_args) => {
            ...
            match process.tuple_from_slice(&[tag, fun_args]) {
                Ok(reason) =>
                    Exception::Runtime(self::error(reason, None, location, None)),
                Err(err) =>
                    err.into(),
            }
        }
...
pub fn undef(process: &Process, m: Term, f: Term, a: Term, location: Location, stacktrace_tail: Term) -> Exception {
    let reason = atom("undef");
    // I'm not sure what this final empty list holds
    match process.tuple_from_slice(&[m, f, a, Term::NIL /*?*/]) {
        Ok(top) => {
            match process.cons(top, stacktrace_tail) {
...
#[inline]
pub fn raise(class: super::Class, reason: Term, location: Location, stacktrace: Option<Term>) -> RuntimeException {
    use super::Class;
...
#[inline]
pub fn error(reason: Term, args: Option<Term>, location: Location, stacktrace: Option<Term>) -> RuntimeException {
    use super::Error;
