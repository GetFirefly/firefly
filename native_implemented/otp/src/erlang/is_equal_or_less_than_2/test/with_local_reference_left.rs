...
fn with_lesser_local_reference_right_returns_false() {
    is_equal_or_less_than(|_, process| process.reference(0), false);
}

#[test]
fn with_same_value_local_reference_right_returns_true() {
    is_equal_or_less_than(|_, process| process.reference(1), true);
}

#[test]
fn with_greater_local_reference_right_returns_true() {
    is_equal_or_less_than(|_, process| process.reference(2), true);
}
...
{
    super::is_equal_or_less_than(|process| process.reference(1), right, expected);
}
