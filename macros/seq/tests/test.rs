...
fn test_range_with_arithmetic() {
    seq!(N in 0..(1 + 2) { struct RangeWithArithmetic~N; });
    let _ = RangeWithArithmetic0;
    let _ = RangeWithArithmetic1;
    let _ = RangeWithArithmetic2;
}

#[test]
fn test_range_with_arithmetic_nested() {
    let tuples = seq!(N in 0..=1 {
        (#(seq!(M in 0..=(N + 1) { (#(M,)*) }),)*)
    });
    assert_eq!(tuples, ((0, 1), (0, 1, 2)));
}
