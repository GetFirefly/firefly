...
fn with_small_integer_zero_errors_badarith() {
    with(|dividend, mut process| {
        ...
        assert_badarith!(erlang::rem_2(dividend, divisor, &mut process))
    })
}
...
fn with_small_integer_divisor_with_underflow_returns_small_integer() {
    with(|dividend, mut process| {
        ...
        let result = erlang::rem_2(dividend, divisor, &mut process);
...
fn with_big_integer_divisor_with_underflow_returns_small_integer() {
    with_process(|mut process| {
        let dividend: Term = (crate::integer::small::MAX + 2).into_process(&mut process);
...
fn with_big_integer_divisor_without_underflow_returns_big_integer() {
    with_process(|mut process| {
        ...
        let divisor: Term = (crate::integer::small::MAX + 2).into_process(&mut process);

        assert_eq!(erlang::rem_2(dividend, divisor, &mut process), Ok(dividend));
    })
}
