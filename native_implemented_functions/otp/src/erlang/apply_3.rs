...
pub fn export() {
    lumen_runtime::code::export::insert(super::module(), function(), ARITY, get_code());
}
...
/// `lumen_runtime::code::export::insert` or returns `undef` exception.
pub fn code(arc_process: &Arc<Process>) -> code::Result {
    ...
    match lumen_runtime::code::export::get(&module_atom, &function_atom, arity) {
        Some(code) => {
            arc_process.stack_popn(3);

            lumen_runtime::code::export::place_frame_with_arguments(
                &arc_process,
                Placement::Replace,
                module_atom,
                function_atom,
