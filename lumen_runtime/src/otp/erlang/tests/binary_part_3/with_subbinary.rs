...
fn without_integer_start_without_integer_length_errors_badarg() {
    with_process(|process| {
        ...
        let binary = Term::subbinary(original, 0, 7, 2, 1, &process);
...
fn with_start_greater_than_size_with_non_negative_length_errors_badarg() {
    with_process(|process| {
        let original = Term::slice_to_binary(&[0b0000_00001, 0b1111_1110, 0b1010_1011], &process);
        let binary = Term::subbinary(original, 0, 7, 0, 1, &process);
...
fn with_start_less_than_size_with_positive_length_past_end_errors_badarg() {
    with_process(|process| {
        let original = Term::slice_to_binary(&[0b0000_00001, 0b1111_1110, 0b1010_1011], &process);
        let binary = Term::subbinary(original, 0, 7, 1, 1, &process);
        let start = 0.into_process(&process);
...
fn with_zero_start_and_byte_count_length_returns_new_subbinary_with_bytes() {
    with_process(|process| {
        ...
        assert_eq!(
            ...
            Ok(Term::subbinary(original, 0, 7, 2, 0, &process))
        );
    });
}
...
fn with_positive_start_and_negative_length_returns_subbinary() {
    with_process(|process| {
        let original = Term::slice_to_binary(&[0b0000_00001, 0b1111_1110], &process);
        let binary = Term::subbinary(original, 0, 7, 1, 0, &process);
        let start = 1.into_process(&process);
        let length = (-1isize).into_process(&process);
