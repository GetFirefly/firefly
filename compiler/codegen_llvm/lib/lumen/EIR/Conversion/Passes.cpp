...
extern "C" MLIRPassManagerRef MLIRCreatePassManager(
    ...
    pm->enableIRPrinting(
        ...
        /*printAfterOnlyOnChange=*/options->printAfterOnlyOnChange,
        llvm::errs(), printerFlags);

    // Configure Pass Timing
    if (options->enableTiming) {
        auto config = std::make_unique<PassManager::PassTimingConfig>(
            PassDisplayMode::List);
