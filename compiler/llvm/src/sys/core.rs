...
extern "C" {
    ...
    /// Returns the module flags as an array of flag-key-value triples.  The caller is responsible
    /// for freeing this array by calling LLVMDisposeModuleFlagsMetadata.
...
extern "C" {
    ...
    ///
    /// Returns NULL if the iterator was already at the beginning and there are no previous global
    /// aliases.
...
extern "C" {
    ...
    ///
    /// The Handlers parameter should point to a pre-allocated array of LLVMBasicBlockRefs at least
    /// LLVMGetNumHandlers() large. On return, the first LLVMGetNumHandlers() entries in the array
    /// will be populated with LLVMBasicBlockRef instances.
