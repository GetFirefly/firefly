...
fn with_empty_list_returns_empty_binary() {
    with_process(|mut process| {
        assert_eq!(
            erlang::list_to_bitstring_1(Term::EMPTY_LIST, &mut process),
...
fn with_subbinary_errors_badarg() {
    ...
    assert_badarg!(erlang::list_to_bitstring_1(subbinary_term, &mut process));
}
    ...
    I: FnOnce(&mut Process) -> Term,
{
    super::errors_badarg(|mut process| {
        erlang::list_to_bitstring_1(iolist(&mut process), &mut process)
    })
}
