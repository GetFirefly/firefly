...
void forAllTraceUses(OpBuilder &builder, Location loc, Value root,
    ...
    for (OpOperand &use : root.getUses()) {
        ...
        // If the trace is passed to a branch/conditional branch, then we need
        // to follow it into the successors and update usages that we find in
        // those blocks. While we're doing so, we ensure the type is correct
        if (auto brOp = dyn_cast_or_null<BranchOp>(user)) {
            ...
            if (index > numTrueOperands) {
                ...
                BlockArgument arg =
                    successor->getArgument(index - numTrueOperands);
                arg.setType(builder.getType<TraceRefType>());
                forAllTraceUses(builder, loc, arg, traceTerm, depth + 1);
            } else {
        ...
        // At this point, we have an operation that either uses the trace as a
        // term, and so requires construction, or is itself a trace constructor
        // (which expects the trace reference). In both cases, if a trace term
        // was constructed already, we need to determine if that construction
        // dominates this use, and if so, use it instead of the raw trace ref.
        // If it doesn't dominate this use, then we need to either insert a
        // constructor, or leave the constructor we've encountered in place.
        //
        // NOTE: This is not complete, we are not doing proper dominance
        // analysis, and I'm waiting to hit a case where we're generating code
        // that requires it before doing something that expensive in a
        // canonicalization pass, since we'd want to reuse the results of that
        // analysis as much as possible. In short, if we've already constructed
        // a trace term, I'm essentially assuming that it dominates all
        // following uses we encounter, and this code is basically just trying
        // to validate that.
        ...
        // is a term, not a raw trace reference, so either construct the term
        // right at the use, or use the result of a constructor that was already
        // present
        if (traceTerm && traceTermDominatesUse) {
            use.set(traceTerm);
        } else {
