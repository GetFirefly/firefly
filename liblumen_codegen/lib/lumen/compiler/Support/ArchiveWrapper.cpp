...
LLVMLumenArchiveIteratorNew(LLVMLumenArchiveRef lumenArchive) {
...
LLVMLumenArchiveIteratorNext(LLVMLumenArchiveIteratorRef RAI) {
  if (RAI->Cur == RAI->End)
    return nullptr;
...
extern "C" const char *
LLVMLumenArchiveChildName(LLVMLumenArchiveChildConstRef Child, size_t *Size) {
...
extern "C" LLVMLumenArchiveMemberRef
LLVMLumenArchiveMemberNew(char *Filename, char *Name,
...
extern "C" LLVMLumenResult
LLVMLumenWriteArchive(char *Dst, size_t NumMembers,
                     const LLVMLumenArchiveMemberRef *NewMembers,
  ...
  if (!Result)
    return LLVMLumenResult::Success;
