...
class ModuleBuilder {
   public:
    ModuleBuilder(MLIRContext &context, StringRef name, Location loc,
                  ...
                  llvm::Triple::ArchType archType, unsigned immediateBitWidth);
    ~ModuleBuilder();
    ...
    void build_if(Location loc, Value value, Block *yes, Block *no,
    ...
    void build_static_invoke(Location loc, StringRef target,
                             ArrayRef<Value> args, bool isTail, Block *ok,
    ...
    Value build_constant_binary(Location loc, StringRef value, uint64_t header,
                                uint64_t flags);
    Attribute build_binary_attr(StringRef value, uint64_t header,
                                uint64_t flags);
