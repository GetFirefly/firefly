...
fn with_empty_list_returns_binary() {
    with(|head, mut process| {
        ...
        assert_eq!(
            ...
            Ok(Term::slice_to_binary(&[0b0000_0001], &mut process))
        );
    })
}
...
fn with_proper_list_returns_binary() {
    with(|head, mut process| {
        ...
        assert_eq!(
            erlang::list_to_bitstring_1(iolist, &mut process),
            Ok(Term::slice_to_binary(&[1, 2], &mut process))
        );
    })
}
...
fn with_heap_binary_returns_binary() {
    with(|head, mut process| {
        ...
        assert_eq!(
            erlang::list_to_bitstring_1(iolist, &mut process),
            Ok(Term::slice_to_binary(&[1, 2, 3], &mut process))
        );
    })
}
    ...
    F: FnOnce(Term, &mut Process) -> (),
{
    with_process(|mut process| {
        let original = Term::slice_to_binary(&[0b0000_0001], &mut process);
        let head = Term::subbinary(original, 0, 0, 1, 0, &mut process);
