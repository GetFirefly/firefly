...
use libeir_ir::{AtomTerm, AtomicTerm, BigIntTerm, BinaryTerm, FloatTerm, IntTerm, NilTerm};
...
impl AsValueRef for AtomicTerm {
    fn as_value_ref(&self, builder: ModuleBuilderRef, options: &Options) -> Result<ValueRef> {
        match self {
            ...
            AtomicTerm::Nil => {
                let val = unsafe { MLIRBuildConstantNil(builder) };
...
impl AsValueRef for ConstMap {
    fn as_value_ref(&self, builder: ModuleBuilderRef, options: &Options) -> Result<ValueRef> {
        let val =
            unsafe { MLIRBuildConstantMap(builder, self.0.as_ptr(), self.0.len() as libc::c_uint) };
...
impl AsValueRef for i64 {
    fn as_value_ref(&self, builder: ModuleBuilderRef, options: &Options) -> Result<ValueRef> {
        let val = unsafe { MLIRBuildConstantInt(builder, *self) };
...
impl AsValueRef for BigIntTerm {
    fn as_value_ref(&self, builder: ModuleBuilderRef, options: &Options) -> Result<ValueRef> {
        ...
        let val = unsafe { MLIRBuildConstantBigInt(builder, s.as_ptr(), width as libc::c_uint) };
...
impl AsValueRef for f64 {
    fn as_value_ref(&self, builder: ModuleBuilderRef, options: &Options) -> Result<ValueRef> {
        let val = unsafe { MLIRBuildConstantFloat(builder, *self) };
...
impl AsValueRef for Symbol {
    fn as_value_ref(&self, builder: ModuleBuilderRef, options: &Options) -> Result<ValueRef> {
        ...
        let val = unsafe { unsafe { MLIRBuildConstantAtom(builder, s.as_ptr(), i) } };
...
impl AsValueRef for BinaryTerm {
    fn as_value_ref(&self, builder: ModuleBuilderRef, options: &Options) -> Result<ValueRef> {
        ...
        let val = unsafe { MLIRBuildConstantBinary(builder, ptr, len, header, flags) };
        if val.is_null() {
            let flags = unsafe {
                if pointer_width == 32 {
...
impl AsAttributeRef for AtomicTerm {
    fn as_attribute_ref(
        ...
        match self {
            ...
            AtomicTerm::Nil => {
                let val = unsafe { MLIRBuildNilAttr(builder) };
                unwrap!(val, "failed to construct constant nil")
            }
        }
    }
...
impl AsAttributeRef for ConstMap {
    fn as_attribute_ref(
        ...
        let val =
            unsafe { MLIRBuildMapAttr(builder, self.0.as_ptr(), self.0.len() as libc::c_uint) };
...
impl AsAttributeRef for i64 {
    fn as_attribute_ref(
        ...
        let val = unsafe { unsafe { MLIRBuildIntAttr(builder, *self) } };
...
impl AsAttributeRef for BigIntTerm {
    fn as_attribute_ref(
        ...
        let width = bi.bits();
        let s = CString::new(bi.to_str_radix(10)).unwrap();
        let val = unsafe { MLIRBuildBigIntAttr(builder, s.as_ptr(), width as libc::c_uint) };
...
impl AsAttributeRef for f64 {
    fn as_attribute_ref(
        ...
        let val = unsafe { MLIRBuildFloatAttr(builder, *self) };
        unwrap!(val, "failed to construct constant float ({})", self)
    }
}
...
impl AsAttributeRef for Symbol {
    fn as_attribute_ref(
        ...
        let i = self.as_usize() as u64;
        let s = CString::new(self.as_str().get()).unwrap();
        let val = unsafe { unsafe { MLIRBuildAtomAttr(builder, s.as_ptr(), i) } };
...
impl AsAttributeRef for BinaryTerm {
    fn as_attribute_ref(
        ...
        let ptr = slice.as_ptr() as *const libc::c_char;
        let len = slice.len() as libc::c_uint;
        let val = unsafe { MLIRBuildBinaryAttr(builder, ptr, len, header, flags) };
        if val.is_null() {
            let flags = unsafe {
                if pointer_width == 32 {
                    BinaryFlags::from_u32(flags as u32)
