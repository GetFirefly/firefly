...
-spec subtract(List1, List2) -> List3
                  when
                      List1 :: [T],
                      List2 :: [T],
                      List3 :: [T],
...
-spec prefix(List1, List2) -> boolean()
                when List1 :: [T], List2 :: [T], T :: term().
...
-spec suffix(List1, List2) -> boolean()
                when List1 :: [T], List2 :: [T], T :: term().
...
suffix(Suffix, List) ->
    ...
    Delta >= 0
    andalso
    nthtail(Delta, List) =:= Suffix.

-spec droplast(List) -> InitList
                  when List :: [T, ...], InitList :: [T], T :: term().
...
seq(First, Last, Inc)
    ...
    if
        Inc > 0, First - Inc =< Last;Inc < 0, First - Inc >= Last ->
...
-spec delete(Elem, List1) -> List2
                when Elem :: T, List1 :: [T], List2 :: [T], T :: term().
...
-spec flatten(DeepList) -> List
                 when DeepList :: [term() | DeepList], List :: [term()].
...
-spec flatlength(DeepList) -> non_neg_integer()
...
-spec rkeymerge(pos_integer(), [X], [Y]) -> [R]
                   when X :: tuple(), Y :: tuple(), R :: tuple().
...
-spec rukeymerge(pos_integer(), [X], [Y]) -> [X | Y]
                    when X :: tuple(), Y :: tuple().
...
keymap(Fun, Index, [Tup|Tail]) ->
    [setelement(Index, Tup, Fun(element(Index, Tup)))|
     keymap(Fun, Index, Tail)];
keymap(Fun, Index, [])
    when is_integer(Index), Index >= 1, is_function(Fun, 1) ->
    [].
...
mapfoldr(F, Accu0, [Hd|Tail]) ->
    ...
    {R,Accu2} = F(Hd, Accu1),
    {[R|Rs],Accu2};
mapfoldr(F, Accu, []) when is_function(F, 2) ->
    {[],Accu}.
...
-spec join(Sep, List1) -> List2
              when Sep :: T, List1 :: [T], List2 :: [T], T :: term().
...
keymergel(I, [T1,[H2|T2]|L], Acc, asc) ->
    keymergel(I,
              L,
              [keymerge2_1(I, T1, element(I, H2), H2, T2, [])|Acc],
              asc);
...
rkeymergel(I, [[H2|T2],T1|L], Acc, asc) ->
    rkeymergel(I,
               L,
               [rkeymerge2_1(I, T1, element(I, H2), H2, T2, [])|Acc],
               asc);
...
ukeymergel(I, [[H1|T1],T2|L], Acc) ->
    ukeymergel(I,
               L,
               [ukeymerge2_2(I, T1, element(I, H1), H1, T2, [])|Acc]);
ukeymergel(_I, [L], []) ->
    L;
ukeymergel(I, [L], Acc) ->
...
rukeymergel(I, [[H2|T2],T1|L], Acc) ->
    rukeymergel(I,
                L,
                [rukeymerge2_1(I, T1, element(I, H2), T2, [], H2)|Acc]);
rukeymergel(I, [L], Acc) ->
