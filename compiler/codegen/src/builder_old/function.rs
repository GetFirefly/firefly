...
impl FunctionBuilder {
    pub fn build(builder: &mut EirBuilder<'b>, module_info: &mut ModuleInfo, func: &ir::FunctionDefinition, options: &Options) -> Result<()> {
        let f = func.function();
        let ident = f.ident();
        {
            module_info.atoms_mut().insert(ident.name.name);
        }
        ...
        for val in f.iter_constants() {
            if let ConstKind::Atomic(AtomicTerm::Atom(AtomTerm(s))) = value_to_const_kind(f, *val) {
                ...
                module_info.atoms_mut().insert(*s);
            }
        }

        // Start building function and any closures defined within
        ...
        for (index, (entry_block, func_entry)) in analysis.func_tree.functions.iter().enumerate() {
            ...
            let ident = if entry_block == root_block {
                ...
                module_info.atoms_mut().insert(fi.name.name);
                ident
            };
            let sfb = ScopedFunctionBuilder::new(builder, module_info, ident, f, &analysis, func_entry, options)?;
            sfb.build()?;
        }
...
pub struct ScopedFunctionBuilder<'f, 'o, 'm> {
    func: Function,
    eir: &'f ir::Function,
    mlir: FuncOp,
    analysis: &'f LowerData,
    builder: &'m mut EirBuilder<'m>,
    module_info: &mut 'm ModuleInfo,
    options: &'o Options,
}
impl ScopedFunctionBuilder<'f, 'o, 'm> {
    pub fn new(builder: &mut EirBuilder<'b>, module_info: &mut ModuleInfo, name: FunctionIdent, eir: &'f ir::Function, analysis: &'f LowerData, func_entry: &'f FunctionEntry, options: &'o Options) -> Self {
        ...
        {
            let term_type = builder.get_term_type();
            if is_closure {
                // Remove the return/escape continuation parameters,
                // and add one parameter, the closure env
                signature.inputs.reserve(entry_args.len() - 2 + 1);
                signature.inputs.push(builder.get_box_type());
            } else {
                // Remove the return/escape continuation parameters
                signature.inputs.reserve(entry_args.len() - 2);
            }
            for arg in entry_args.iter().skip(2).copied() {
                signature.inputs.push(term_type);
            }
            signature.outputs.push(term_type);
        }

        // Construct the parameter list for the entry
        let mut entry_params = Vec::with_capacity(signature.inputs.len() + 1);
        if is_closure {
            entry_params.push(None);
        }
        for v in entry_args.iter().skip(2).copied() {
            entry_params.push(Some(v))
        }

        // Create function
        let span = eir.span();
        let mut func = Function::with_name_signature(span, name, signature, Visibility::Public);
        let (mlir, entry_block) = func.build(builder, module_info)?;

        // Mirror the entry block for our init block
        let init_block =
            func.new_block_with_params(Some(func_entry.entry), entry_block, &entry_params);
        // Initialize ret/esc continuations
        func.set_return_continuation(ret, init_block);
        func.set_escape_continuation(esc, init_block);

        let pos = Position::at(init_block);
...
impl ScopedFunctionBuilder<'f, 'o, 'm> {
    ...
    #[inline]
    pub fn live_at(&self, block_handle: Block) -> BoundSet<'f, ir::Value, ()> {
        let ir_block = self.func.block_to_ir_block(block_handle).unwrap();
    ...
    #[inline]
    pub fn live_in(&self, block_handle: Block) -> BoundSet<'f, ir::Value, ()> {
        let ir_block = self.get_ir_block(block_handle);
    ...
    #[inline]
    pub fn is_live_at(&self, block_handle: Block, value_handle: Value) -> bool {
        let ir_block = self.get_ir_block(block_handle);
        let ir_value = self.get_ir_value(value_handle);
    ...
    /// Panics if either the value or block do not have EIR representation
    #[inline]
    pub fn is_live_in(&self, block_handle: Block, value_handle: Value) -> bool {
        let ir_block = self.get_ir_block(block_handle);
        let ir_value = self.get_ir_value(value_handle);
    ...
    pub fn get_ir_block(&self, block_handle: Block) -> ir::Block {
        self.func.block_to_ir_block(block_handle).unwrap()
    }
    ...
    #[inline]
    pub fn get_ir_value(&self, value_handle: Value) -> ir::Value {
        self.func.value_to_ir_value(value_handle).unwrap()
    }
    ...
    pub fn value_location(&self, ir_value: ir::Value) -> Location {
        if let Some(loc) = self.maybe_value_location(ir_value, None) {
            return loc;
        }

        log::warn!("unknown location: {:?}", self.value_kind(ir_value));
        self.module_info.location_from_index(self.func.span).unwrap()
    }
    ...
    pub fn maybe_value_location(
        ...
        if let Some(locs) = self.eir.value_locations(ir_value) {
            let mut fusing = Vec::with_capacity(locs.len());
            for loc in locs.iter().copied() {
                if let Some(l) = self.module_info.location_from_index(loc.start().index()) {
                    fusing.push(l);
                }
            }
            match fusing.len() {
                1 => fusing.pop(),
                n if n > 1 => Some(self.builder.get_fused_loc(&fusing)),
                _ => default
            }
        } else {
            default
    ...
    pub fn block_location(&self, ir_block: ir::Block) -> Location {
    ...
    pub fn maybe_block_location(
        ...
        let mut fusing = Vec::with_capacity(locs.len());
        for loc in locs.iter().copied() {
            if let Some(l) = self.module_info.location_for_index(loc.start().index()) {
                fusing.push(l);
            }
        }
        match fusing.len() {
    ...
    pub fn is_block_argument(&self, ir_value: ir::Value, block_handle: Block) -> bool {
        for arg in self.eir.block_args(self.get_ir_block(block_handle)) {
            if ir_value == *arg {
                return true;
            }
        }
...
impl ScopedFunctionBuilder<'f, 'o, 'm> {
    ...
    pub fn block_data(&self, block_handle: Block) -> &BlockData {
        self.func.block_data(block_handle)
    }

    /// Returns the MLIR block reference for the given block
    #[inline]
    pub fn block_ref(&self, block_handle: Block) -> mlir::ir::Block {
        self.func.block_to_block_ref(block_handle)
    }

    /// Returns the value data for the given value
    #[inline]
    pub fn value_data(&self, value_handle: Value) -> &ValueData {
        self.func.value_data(value_handle)
    }

    /// Returns the MLIR value reference for the given value
    #[inline]
    pub fn value_ref(&self, value_handle: Value) -> ValueRef {
        self.func.value_to_value_ref(value_handle)
    }
    ...
    #[inline]
    pub fn new_value(&mut self, ev: Option<ir::Value>, mv: mlir::ir::Value, data: ValueDef) -> Value {
    ...
    #[inline]
    pub fn block_args(&self, block_handle: Block) -> Vec<BlockArgument> {
        let block = self.func.block_data(block_handle).block;
        let num_args = block.num_arguments();
        let result = Vec::with_capacity(num_args);
        for i in 0..num_args {
            result.push(block.get_argument(i).unwrap());
        }
        result
    }
...
impl ScopedFunctionBuilder<'f, 'o, 'm> {
    /// Builds the function
    pub fn build(mut self, func_entry: &FunctionEntry) -> Result<()> {
    ...
    fn unpack_closure_env(
        ...
        let loc = self.module_info.location_from_index(self.func.span.start_index());
        // Get the closure env argument as a closure type pointer
        let env_arg = self
            .block_args(entry)
            .get(0)
            .copied()
            .expect("expected closure env argument from block");
        let closure_type = self.builder.get_closure_type(Some(num_values));
        let closure_ptr_type = self.builder.get_ptr_type(closure_type);
        let env = self.builder.build_cast(loc, env_arg, closure_ptr_type);
        // For each env value, unpack the value and store a reference to it
        // for use later
        let num_values = live_at.iter().count();
        let mut values: Vec<mlir::ir::Value> = Vec::with_capacity(num_values);
        for i in 0..num_values {
            let op = self.builder.build_unpack_env(loc, env, i);
            values.push(op.result());
        }
        // Bind original EIR values to their unpacked counterparts in MLIR
        for (i, (ir_value, value)) in live_at.iter().zip(values.iter()).enumerate() {
            debug_in!(self, "env value mapped: {:?} => {:?}", ir_value, value);
            self.new_value(Some(ir_value), *value, ValueDef::Env(i));
        }
    ...
    fn create_block(
        ...
        let block_ref = unsafe {
            MLIRAppendBasicBlock(self.builder, self.mlir, params_ptr, params.len() as c_uint)
        };
...
pub(super) fn get_block_argument(block_ref: BlockRef, index: usize) -> ValueRef {
    let value_ref = unsafe { MLIRGetBlockArgument(block_ref, index as c_uint) };
